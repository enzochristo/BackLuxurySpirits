...
class LoginAdminUseCase:
    ...
    def execute(self, login_DTO:LoginDTO, response:Response, request:Request):
        ...
        if (user.type != "admin"):
             ...
             return {"status": "error", "message": "Apenas administradores têm acesso a este login"}
        
        token = jwt.encode({"email": user.email, "id": str(user.id), "user_type": user.type}, os.getenv("USER_JWT_SECRET"))
